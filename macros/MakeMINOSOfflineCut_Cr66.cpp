...
#include <TFitter.h>
#include <time.h>
#include "/usr/local/root/root_v5.34.18/include/Math/Vector3D.h"
#include "/home/gamma14/exp/minos/liboffline/TMinosClust.h"
#include "/home/gamma14/exp/minos/liboffline/TMinosResult.h"
#include "TCutG.h"
...
//  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if(argv[2]==NULL) {
	char*  pch;
    	...
    	strcat(OutFile, pch2);
    	strcat(OutFile, "_MINOS66Cr_newVdriftFe72.root");
  ...
  //--------------------------------------------------------------------
  //PID Cuts
  TFile *BRcut = new TFile("../cut/Cr66/brcut_67Mn_new.root","READ");
  TFile *ZDcut = new TFile("../cut/Cr66/zdcut_66Cr_new.root","READ");
  TCutG *brcut;
  TCutG *zdcut;
  ...
  ifstream ConfigFile;
  ConfigFile.open("./../configs/ConfigFileSetupCr66.txt");
  string Header;
  ...
  cout << " Electronics        :::   MINOSthresh = " << MINOSthresh << " (bins)  ;  TimeBinElec = " << TimeBinElec << " (ns)  ;  VDrift = " << VDrift << " (cm/micros)  ;  Tshaping = " << Tshaping << " (ns)  ;  DelayTrig = " << DelayTrig << " (ns)" << endl;
...
//	Filter_canvas.clear();
	...
	bool zdCutBool = false;
    	...
    	//if( zdcut->IsInside(beam_zd_911->GetAoQ(),beam_zd_911->GetZet()) ) zdCutBool = true;
    	if( zdcut->IsInside(beam_zd_911->GetAoQ()-0.00005*F11X+0.00006*F9X-0.0001*F11A+0.001*F9A,beam_zd_911->GetZet()) ) zdCutBool = true;
...
//             		y_vertex = parFit_1[2]+parFit_1[3]*z_vertex;
	...
	//6:MINOS://////////// Calculate the beta corresponding to z_vertex found ////////////:MINOS://
			...
			//Target starts 11 mm upstream the MM detector
			if(z_vertex>=-20 && z_vertex<=100) beta = beta1 - (z_vertex-11.0)*(beta1-beta2)/TargetLength;
			//else if(z_vertex>100 && z_vertex<=120) beta = beta2;
			//else beta = betam;
