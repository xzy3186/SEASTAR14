...
// ./runAnalysisMINOSDALI -12 12 15 0.03 0.049 
// with ./runAnalysisMINOSDALI timeCutLow timeCutHigh maxaddbackdistance betaDiffLow betaDiffHigh
#include "Event.h"
...
int main(int argc, char **argv) {
  ...
  EventMINOS *TreeMINOS = new EventMINOS();
  
  Long64_t nentries = Tree->fChain->GetEntriesFast();
  Long64_t nentriesMINOS = TreeMINOS->fChainMINOS->GetEntriesFast();

  if( (nentries - nentriesMINOS)!=0 )  cout << " Problem : Different number of events, AllTree=" << nentries << " , MINOS=" << nentriesMINOS << endl;
  ...
  TH1F *h_beta_vertex = new TH1F("h_beta_vertex","h_beta_vertex",100,0,1);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(fDali[0].e>0) SortData(fDaliFold,fDali);    //Sort DALI2 crystals by decreasing energies
    ...
    */
    ...
    for(int j=0;j<fDaliMultTa;j++){
      h_doppler[3+counter*10]->Fill(fDali[j].idwa,fDali[j].doppwa[counter]);				//all mult
