...
void gg_analysis(){
   ...
   sim[2] = new TFile("./geant4spec/78cu_h_77ni_700.root");
   ...
   for(int i=0; i<5; i++){
      ...
      hsim[i] = new TH1D(temp,temp,200,0,5000);
   ...
   // fit
   TH1D *htmp[200];
   for(int bin=1;bin<=180;bin++){ // 25 keV/bin from 0 to 5000 keV
      sprintf(temp,"htmp_%i",bin-1);
      htmp[bin] = new TH1D(temp,temp,100,0.,5000.);
      hh->ProjectionY(temp,bin,bin+19);
      if(htmp[bin]->Integral(1,100)<=10){
         for(int i=0; i<5; i++){
            //hgg[i]->SetBinContent(bin, 0);
            hgg[i]->SetPoint(bin-1, (bin-1)*25+12.5, 0);
         }
      }else{
         htmp[bin]->Sumw2();
         ...
         for(int i=0; i<5; i++){
            ...
            hgg[i]->SetPoint(bin-1, (bin-1)*25+12.5, whole1->GetParameter(i));
         }
      }
